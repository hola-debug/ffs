-- ============================================
-- FFS.FINANCE - NUEVA ARQUITECTURA DE BOLSAS
-- ============================================
-- Sistema basado en BOLSAS DE DINERO que se separan del ingreso
...
-- Cuentas del usuario (efectivo, banco, billetera)
...
-- Categorías para movimientos
CREATE TABLE categories (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  type TEXT NOT NULL CHECK (type IN ('income', 'fixed_expense', 'saving', 'pocket_expense')),
  icon TEXT,
  color TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(user_id, name)
...
-- ============================================
-- BOLSAS (POCKETS) - CONCEPTO PRINCIPAL
-- ============================================
...
-- ============================================
-- MOVIMIENTOS (MOVEMENTS) - Reemplaza transactions
-- ============================================
...
-- Trigger para updated_at en pockets
...
-- Función para inicializar categorías por defecto para un usuario
